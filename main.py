...
# --- ÇEREZ YÖNETİCİSİ ---
cookie_manager = stx.CookieManager(key="auth_mgr_v48_5")
...
# --- GELİŞMİŞ PDF MOTORU (KARAKTER DÜZELTİCİ) ---
def clean_text_for_pdf(text):
    # 1. LaTeX Temizliği (Gerekirse)
    text = text.replace(r'\[', '').replace(r'\]', '').replace(r'\(', '').replace(r'\)', '')
    
    # 2. Karakter Değişimi (PDF'in Anlayacağı Dile Çevir)
    replacements = {
        # Türkçe
    ...
    # ASCII dışı karakterleri temizle (Çökmemesi için)
...
def create_safe_pdf(title, content):
    ...
    # Unicode yoksa temizlenmiş metni kullan
    if not use_unicode:
            ...
            if st.session_state.logged_in:
                if get_credit(st.session_state.username) > 0:
                    ...
                    with st.spinner("Hazırlanıyor..."):
                        ...
                        GÖREV: Detaylı anlat. 1000 Kelime. 3 Örnek çöz. Sembolleri (√, ², π) DOĞRUDAN kullan.
                        """
                        try:
            ...
            if st.session_state.logged_in:
                if get_credit(st.session_state.username) > 0:
                    ...
                    with st.spinner("Yazılıyor..."):
                        ...
                        TALİMAT: Soruları 1., 2. diye numaralandır. En alta CEVAP ANAHTARI koy.
                        Sembolleri (√, ², π) doğrudan kullan.
                        """
                        try:
