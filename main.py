...
# --- ÇEREZ YÖNETİCİSİ ---
cookie_manager = stx.CookieManager(key="auth_mgr_v52")
...
# ==========================================
# YAN MENÜ (SADECE GEREKLİ OLANLAR)
# ==========================================
with st.sidebar:
    st.title("🗂️ Menü")
        ...
        if can_proceed:
            with st.spinner(random.choice(["Hoca bakıyor...", "Çözülüyor..."])):
                try:
                    ...
                    # RESİMLİ KAYIT (DÜZELTİLDİ)
                    if st.session_state.logged_in:
                        resim_kayit = base64_image if gorsel_veri else None
                        kayit_soru = "Fotoğraflı Soru" if gorsel_veri else metin_sorusu
                        save_history(st.session_state.username, kayit_soru, cevap, resim_kayit)
