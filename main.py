...
# --- ÇEREZ YÖNETİCİSİ (v101) ---
cookie_manager = stx.CookieManager(key="auth_mgr_v101")
...
def add_user(username, password):
    try:
        ...
        # Hata detayını terminale yaz, kullanıcıya gösterme
        print(f"DB Error: {e}")
        return False 
...
try:
    ...
    # --- MİSAFİR KİLİDİ (v101) ---
    # Çerez varsa VE aktif bir cevap ekranda yoksa -> KİLİTLE
    has_cookie = "guest_blocked_v101" in cookies
...
with st.sidebar:
    ...
    if st.session_state.logged_in and st.session_state.username == admin_mail:
        ...
        if st.button("Misafir Hakkını Sıfırla"):
            try: cookie_manager.delete("guest_blocked_v101"); st.rerun()
            except: pass
...
if not st.session_state.logged_in:
    if st.session_state.guest_locked:
        ...
        try:
            if "guest_blocked_v101" in cookie_manager.get_all():
...
if guest_blocked:
    ...
    # 🛑 MİSAFİR KİLİDİ (CEVAP GÖRÜNDÜĞÜ AN ÇEREZ AT)
    if not st.session_state.logged_in:
        try: cookie_manager.set("guest_blocked_v101", "true", expires_at=datetime.datetime.now() + datetime.timedelta(days=30))
        except: pass

    if st.button("⬅️ Yeni Soru"):
        st.session_state.son_cevap = None
        # Misafirsen "Yeni Soru" deyince kilitle
        if not st.session_state.logged_in:
             st.session_state.guest_locked = True
        st.rerun()
